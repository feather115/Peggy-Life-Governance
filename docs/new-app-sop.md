...
這份文件是照 `apps/calorie-tracker`、`apps/recipe-book`、`apps/calendar` 三個現有 app
的慣例整理出來的。**2026-07-05 做過一輪三 app 架構整合**，把原本各自漂移的部分收斂成同
一套框架（LINE 前端邏輯抽進 `packages/shared/lineAuth`、LINE api 檔案逐字同步、db.js 錯
誤處理規則統一、supabase client 單例化），所以現在三個 app 的骨架是刻意保持一致的。
以後要加新 app，照這份 SOP 做就會自動長成同一個樣子，AI 助理接手時也能立刻套用相同慣例，
不用每次重新摸索。
...
## 1.5 `packages/shared` 有什麼（先看這裡，不要重複造輪子）

跨 app 共用的程式碼只有一份，都在 `packages/shared`。新 app 動手寫任何「其他 app 好像
也有」的東西之前，先確認是不是已經在這裡：
...
3. **`vite.config.js` 固定 dev port**，跟其他 app 錯開，並把新 app 加進根目錄
   `.claude/launch.json`（preview 工具靠它啟動）。目前的分配：calorie-tracker `3456`、
   calendar `3457`、recipe-book `5173`。
   ```js
   export default defineConfig({
     plugins: [react()],
     server: { port: <獨一無二的 port> },
   });
   ```
4. 根目錄跑一次 `npm install`。
...
- **`use<Domain>.js`**：唯一的狀態中樞，回傳 `{ ...state, ...actions }` 給元件用；
  ...
  - **初始載入的多個獨立查詢用 `Promise.all` 平行打**，非致命的部分各自 `.catch` 降級
    （回空陣列 + `console.warn`），不要串行 await（參考 calorie-tracker `useAppData.js`
...
- **錯誤碼慣例**：「伺服器自己沒設定好」的錯誤（缺 env var 之類）throw 時掛上
  `statusCode: 500`——`throw Object.assign(new Error('伺服器未設定 XXX'), { statusCode: 500 })`
  ——handler 的 catch 用 `res.status(e.statusCode || 400)` 回應，除錯時才分得出
  「前端送錯東西（400）」和「後端沒設定好（500）」。handler 同時把 supabase 錯誤的
  `code`/`details`/`hint` 一起回給前端並 `console.error` 記 log（LINE 相關 handler 都
  已經這樣寫，照抄即可）
...
- **表定義**：`shared.user_profiles(user_id uuid PK, display_name text, email text,
  ...
  ＋ `2026-07-06_user_profiles_service_role_grant.sql`（後者補 `service_role` 的表權限，
  LINE 首次登入的暱稱 seed 需要，新環境兩支都要跑）
- **LINE 首次登入自動帶入暱稱**：`api/_lineLogin.js` 在使用者還沒設暱稱時，會把 LINE
  顯示名稱 seed 進這張表（僅空白時，不會蓋掉自訂暱稱），三個 app 共同受惠，新 app
  不用另外處理
...
- **`shared` schema 新增表時，`grant` 給 `authenticated` 之外別忘了 `service_role`**——
  service_role 有 BYPASSRLS，但**表層級的 GRANT 還是要有**，漏掉的話 `api/` 裡的 admin
  client 寫入會吃 `permission denied for table ...`（`user_profiles` 踩過，`line_links`
  當初就有記得 grant）
