...
選項庫標籤/備註）與**回顧面**（今天的感覺/可前後排序的＃快速注記/分類標籤），表單裡回顧面可收合、
...
在卡片頂端、標題大字、描述/心情小字、＃注記 pill、分隔線下是分類標籤 chip、地點與以 `, ` 分隔的完整同伴名單；
分類標籤的細節欄位在手機與桌機皆會顯示最近使用的歷史建議，輸入文字後即時篩選；歷史細節可在
「管理分類與標籤」中改名或刪除，並同步更新過去紀錄。
時間軸卡片的標題前不顯示顏色圓點，計時與全天紀錄的標題皆使用相同字級。
週檢視的每日日期標題以淡霧藍、灰藍兩個相近色帶交錯呈現，維持一致藍色調且不過度跳色。
...
4. **Settings → Git → Ignored Build Step** 填：
   ```bash
   git diff --quiet $VERCEL_GIT_PREVIOUS_SHA HEAD -- . ../../packages/shared
   ```
