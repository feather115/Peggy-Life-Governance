// 日檢視：單日紀錄 + 到期任務合併時間軸，可切換前一天/後一天，底部有「新增紀錄」按鈕。
// 卡片渲染在 TimelineItems（三個檢視共用同一套白卡版型），這裡只負責日期導覽、空狀態與新增按鈕。
...
export default function DayView({ dateKey, onShiftDay, recordsByDate, categories, tasksByDueDate, onEdit, onCreate, onGoToTasks }) {
  ...
  return (
    <div style={S.wrap}>
      ...
      <div style={S.footer}>
        <button type="button" style={S.addBtn} onClick={() => onCreate(dateKey)}>＋ 新增紀錄</button>
      </div>
    </div>
  );
}
