...
- **全天項目的呈現** — 跟計時項目**同一套卡片版型**，只差兩件事：**不顯示時間列**
  ...
  白底才不會被吃掉：日記標籤 chip 走 `DiaryTags onTint`、＃注記 pill 與事件標籤
  chip 直接換 `THEME.surface`（`hashtagBg` 跟 `primarySoft` 剛好同色）。三個檢視
  共用同一份（都在 `TimelineItems`）。
