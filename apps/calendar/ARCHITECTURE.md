...
- **`DayView.jsx`** — 單日事件+日記+任務合併時間軸（`buildDayTimeline`），事件卡片顯示
  ...
  塞進時間欄那格文字寫「全天」——沒有時間欄，直接 dot + 標題起頭，背景換成
  `THEME.primarySoft` 跟計時項目的卡片區分開來，日記標籤 chip 背景也跟著換成
  `THEME.surface`（不然會被同色系的卡片背景吃掉）。這是使用者實際看畫面反饋的：
  「全天」文字塞在時間欄裡看起來很生硬，拆成獨立卡片樣式比較自然。
