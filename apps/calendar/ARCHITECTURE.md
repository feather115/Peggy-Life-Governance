...
- **`DiaryForm.jsx`** — 新增/編輯單筆日記：已選標籤即時預覽、全天切換、時間/結束時間
  ...
  **不含**「管理分類與標籤」入口——那個入口移到設定頁了（見下）。每個分類卡片底下有
  `AddTagControl`（內部元件）：寫日記寫到一半發現想要的標籤還沒建立，可以就地輸入
  新標籤名稱直接建立並選取，不用跳出去設定頁。跟 `ManageTags.jsx` 一樣做「標籤名稱
  跨分類不能重複」的檢查，但這裡的處理方式不一樣——`ManageTags.jsx` 是彈出確認卡片
  問要不要把標籤「移到」這個分類；`AddTagControl` 因為只是要選一個標籤來標記日記，
  不是在管理分類結構，所以直接把那個既有標籤選起來就好（不建立重複、也不搬動
  分類），順便顯示一行提示「「X」已經在「Y」分類，直接幫你選起來了」。
