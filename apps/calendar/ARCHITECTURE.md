...
日記（多筆/天，標籤依分類管理）。資料存在 Supabase 的 `calendar` schema，每人只能看/改
自己的資料（沒有分享機制）。需要登入才能使用，跟 calorie-tracker、recipe-book 共用同一個
Supabase 專案的 `auth.users`，可以用 LIFF 連結在 LINE App 裡直接開啟並自動登入。

視覺風格照 Claude Design 產出的「TY Calendar Design System」設計稿實作（柔和藍主題），
分三階段落地：Phase 1 視覺改版 + 事件顏色/標籤（已完成）、Phase 2 日記功能（已完成）、
Phase 3 週期性任務（未開始）。
...
```
Supabase ⇄ db.js ⇄ useEvents.js / useDiary.js ⇄ Root.jsx / App.jsx ⇄ components/*
          (純 API)  (狀態+動作，兩個獨立 hook)      (登入閘口 / 協調)     (畫面)
```

- 元件**不會**直接呼叫 `db.js`。事件相關資料/動作走 `useEvents()`；日記與標籤分類走
  `useDiary()`。兩個 hook 平行存在，`App.jsx` 同時使用兩者並互相傳遞需要的資料
  （例如 `DayView` 需要 `useEvents` 的事件 + `useDiary` 的日記，合併成一條時間軸）。
- `Root.jsx` 負責：`.env` 檢查、LINE 自動登入、Email/密碼登入閘口。
- `App.jsx` 負責 520px 置中外殼，依 `editing`/`editingDiary`/`managingTags` state
  切換「檢視畫面」或對應表單（互斥，同時只會有一個 overlay）。
...
| **月檢視（格線月曆、事件/日記圓點、選中日摘要卡）** | `src/components/MonthView.jsx` |
| **週檢視（7 天直向列表，含事件+日記時間軸）** | `src/components/WeekView.jsx` |
| **日檢視（事件+日記合併時間軸、新增按鈕）** | `src/components/DayView.jsx` |
...
| **日期運算、事件/日記分組、時間軸合併** | `src/utils.js` |
| **Supabase 查詢（events / diary_entries / tag_categories 的 CRUD）** | `src/db.js` |
...
| **事件狀態中樞（事件清單、檢視模式、月/週/日翻頁與選中日）** | `src/useEvents.js` |
| **日記狀態中樞（日記清單、分類標籤、預設分類種子）** | `src/useDiary.js` |
| **主外殼、editing/editingDiary/managingTags state** | `src/App.jsx` |
...
- **`src/App.jsx`** — 520px 置中外殼，同時載入 `useEvents()` + `useDiary()`，依三個
  editing state（互斥）切換「月/週/日檢視」或對應表單。
- **`src/useEvents.js`** — ⭐ **事件狀態中樞**。載入事件、`eventsByDate`（依日期分組）、
  `view`（月/週/日）、`anchorKey`（目前翻頁翻到哪個月/週/天）、`selectedDateKey`
...
- **`src/db.js`** — Supabase 的純查詢函式：events（`loadEvents`/`createEvent`/…）、
  ...
  （`loadCategories`/`createCategories`/`updateCategory`/`deleteCategory`）。
...
- **`src/utils.js`** — 日期字串轉換（`dateKeyFrom`/`parseDateKey`）、月曆格線
  ...
  `groupDiaryByDate`）、`buildDayTimeline(events, diaryEntries)`（把某天的事件+日記
  合併成一條依時間排序的時間軸，Month/Week/Day 三個檢視共用同一個函式，行為才會一致）、
  `<input type="datetime-local">` 字串轉換。

### 畫面（`src/components/`）
- **`ViewTabs.jsx`** — 月/週/日三個 tab + 「今天」按鈕。
- **`MonthView.jsx`** — 格線月曆，日期下方顯示事件顏色圓點（最多 3 個不同色）+ 日記圓點
  （固定用 `theme.primaryDark`），圖例列說明兩種點代表什麼。點日期只會「選中」
  （`onSelectDay`），不離開月檢視；下方的「選中日摘要卡」顯示該天的合併時間軸，
  點摘要卡標題才會呼叫 `onOpenDay` 跳去日檢視。
- **`WeekView.jsx`** — 一週 7 天直向列表，每天顯示事件+日記合併時間軸，點某一天呼叫
  `onOpenDay` 跳去日檢視。
- **`DayView.jsx`** — 單日事件+日記合併時間軸（`buildDayTimeline`），事件卡片顯示顏色點
  /時間/標題/描述/標籤，日記卡片顯示時間/標籤（依分類上色）/地點/同伴/心情筆記，
  可切換前一天/後一天，底部並排「＋ 新增事件」「＋ 新增日記」兩個按鈕（新增動作只從
  日檢視發起，月/週檢視只負責導覽——見下方原因說明）。
...
**設計重點：**
- **RLS 全開，沒有分享機制** — 三張表的 policy 都是 `auth.uid() = user_id`
...
> 之後有新欄位/新表再依日期新增檔案，格式跟其他 app 一致：`YYYY-MM-DD_描述.sql`。
