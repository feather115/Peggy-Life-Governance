...
新增食物時可在同一個食物庫面板切換「手動輸入 / JSON 輸入」；JSON 匯入會同時存進食物庫並直接加入目前餐別，支援以 `unit` 指定份量（例如 `"unit":"1 份"`），也相容 `serving` 欄位；未提供時預設為 `1 份`。

已結束的減重挑戰預設收在「歷史挑戰」內；展開並選取場次後，可查看結束前的排行榜、頒獎台與每週戰績，並用「回到挑戰」返回進行中的挑戰或歷史清單。
已結束挑戰的建立者也能使用「原班人馬再來一局」，調整新局名稱與日期後，直接讓上一局全部成員加入。
挑戰賽卡片的標題採單行省略，避免名稱稍長時把 Banner 擠成兩行。
挑戰賽把選填的「起始體重 / 當前體重」獨立成 `體重紀錄（選填）` 卡片；會直接保存到挑戰者自己的資料，進入挑戰時就能看到上次填的值，並可用「協助帶入」幫忙換算公斤差值。
