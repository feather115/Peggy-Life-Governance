...
- **料理行事曆優化：新增搜尋篩選與時間排序** (2026-06-28)：
  - 新增「🔍 輸入料理關鍵字搜尋...」輸入欄位，使用者可在選擇料理時輸入文字進行即時搜尋篩選。
  - 當搜尋結果過濾後，系統會自動預先選擇第一個匹配的食譜，按「加入」即可一鍵新增，無須展開下拉選單。
  - 料理選項排序改為依據「最後烹調時間（`last_cooked_at`）」、「編輯時間（`updated_at`）」及「建立時間（`created_at`）」降序排序，讓最近製作或更新的食譜排在最前面，便於快速選取。
  - 當使用者在行事曆中新增或刪除烹調歷史時，系統會自動在前端與資料庫中同步更新對應食譜的「最後製作時間」欄位。
